...
module.exports = class BaseHomeWizardDriver extends Homey.Driver {
  ...
  async onPairListDevices() {
    try {
      ...
      let devices;
      if (deviceType === "sw_mesh_smoke_detector") {
        const deviceModel = this.getDeviceModel();
        devices = homeResponse.data.devices.filter(d => d.type === deviceType && d.model === deviceModel);
      } else if (deviceType === "sw_smoke_detector") {
        devices = homeResponse.data.devices.filter(d => d.type === deviceType || (d.type === "sw_mesh_smoke_detector" && d.model === "rf868_mesh_smoke_detector_siterwell"));
      } else {
        devices = homeResponse.data.devices.filter(d => d.type === deviceType);
      }
